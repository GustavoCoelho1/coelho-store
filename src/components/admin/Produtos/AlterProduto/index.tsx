...
import ImageSelector from 'components/common/ui/ImageSelector';
...
const IMAGEM_BY_LINK = gql`
    query ($link: String!) {
        produtoImagemByLink(link: $link) {
            img_cod
        }
    }
`;

const ATUALIZAR_IMAGEM = gql`
    mutation ($id: ID!, $data: iProdutoImagem!) {
        updateProdutoImagem(id: $id, data: $data) {
            img_cod
        }
    }
`;
...
const ATUALIZAR_PRODUTO = gql`
    mutation ($id: ID!, $data: iProdutoUpdate!) {
        updateProduto(id: $id, data: $data) {
            prod_cod
        }
    }
`;
...
const AlterProdutoForm: React.FC<Props> = ({
    setModalVisible,
    setModalContent,
}) => {
    ...
    const storeProduct = async () => {
        try {
            ...
            const produto = prodData?.updateProduto;
            ...
            imagesList.forEach(async (image) => {
                if (image.link) {
                    ...
                    const { data: imgData } = await GQLClient.mutate({
                        mutation: IMAGEM_BY_LINK,
                        variables: {
                            link: url,
                        },
                    });

                    const thisImg = imgData?.produtoImagemByLink;

                    const { data: imgUpdate } = await GQLClient.mutate({
                        mutation: ATUALIZAR_IMAGEM,
                        variables: {
                            id: thisImg.img_cod,
                            data: {
                                link: url,
                    ...
                    const imagem = imgUpdate?.updateProdutoImagem;
            ...
            if (produto) {
                ...
                setModalContent(
                    <SuccessMessage
                        ...
                        subtitle="Deseja alterar mais produtos?"
    ...
    return (
        ...
        >
            ...
            <Button type="submit" $isColored={true}>
                Alterar
            </Button>
        </form>
    );
...
export default AlterProdutoForm;
